...
# Buffer position
bufferPOS = np.radians([180, -135, 135, -90, -90, 0])

# Store world coordinates of green and yellow blocks
xw_yw_G = []
xw_yw_Y = []

# Define goal positions for each block color
GreenGoal = [0.15,-0.15,0.03]
YellowGoal = [0.3,-0.15,0.03]
...
def main():
    ...
    for i in range(len(stored_G)):
        ...
        print(stored_G[i][1])
        print("END")
        rospy.loginfo("Sending green goal")
        move_block(pub_command, loop_rate, [stored_G[i][0],stored_G[i][1],0.03], np.add(GreenGoal,[0,0,0.03*i]), vel, accel)
        print("Finished goal!")
    
    for j in range(len(stored_Y)):
        print("Objects detected:\n")
        print(len(stored_Y))
        print("START")
        print(stored_Y[j][0])
        print(stored_Y[j][1])
        print("END")
        rospy.loginfo("Sending yellow goal")
        move_block(pub_command, loop_rate, [stored_Y[j][0],stored_Y[j][1],0.03], np.add(YellowGoal,[0,0,0.03*j]), vel, accel)
        print("Finished goal!")
