...
def gripper(pub_cmd, loop_rate, io_0):
    ...
    while(at_goal == 0):
        ...
        if(spin_count >  SPIN_RATE*5):
            ...
            pub_cmd.publish(driver_msg)
            rospy.loginfo("gr Just published again driver_msg")
            spin_count = 0
...
def move_arm(pub_cmd, loop_rate, dest, vel, accel):
	...
	#print("BELOW IS DRIVER_MSG.DESTINATION")
	#print(driver_msg.destination[1])
	#print(driver_msg.destination[1][1])
        ...
        if(spin_count >  SPIN_RATE*5):
            ...
            pub_cmd.publish(driver_msg)
            rospy.loginfo("ma Just published again driver_msg")
            spin_count = 0
...
def move_block(pub_cmd, loop_rate, start_loc, start_height, \
    ...
    start_pos = Q[start_loc-1][start_height-1][0]
    end_pos = Q[end_loc-1][end_height-1][0]

    #move arm to beginning position and grab the block    
...
def main():
    ...
    while(not input_done):
        ...
        start_input = input("Enter starting column <Either 1 2 or 3> ")
        end_input = input("Enter ending column <Either 1 2 or 3> ")
        print("You entered something")

        if(int(input_string) == 1):
            start_col = int(start_input)
            end_col = int(end_input)
            input_done = 1
            loop_count = 1
        ...
        if((start_col == 1 and end_col == 2) or (start_col == 2 and end_col == 1)):
            mid_col = 3
        elif((start_col == 1 and end_col == 3) or (start_col == 3 and end_col == 1)):
            mid_col = 2
        elif((start_col == 2 and end_col == 3) or (start_col == 3 and end_col == 2)):
            mid_col = 1
        else:
            print("Invalid start or end columns")
    ...
    while(loop_count > 0):
         ...
         move_block(pub_command, loop_rate, start_col,3,end_col,1) 

         rospy.loginfo("Sending goal 2 ...")
         move_block(pub_command, loop_rate, start_col,2,mid_col,1)

         rospy.loginfo("Sending goal 3 ...")
         move_block(pub_command, loop_rate, end_col,1,mid_col,2)

         rospy.loginfo("Sending goal 4 ...")
         move_block(pub_command, loop_rate, start_col,1,end_col,1)

         rospy.loginfo("Sending goal 5 ...")
         move_block(pub_command, loop_rate, mid_col,2,start_col,1)

         rospy.loginfo("Sending goal 6 ...")
         move_block(pub_command, loop_rate, mid_col,1,end_col,2)

         rospy.loginfo("Sending goal 7 ...")
         move_block(pub_command, loop_rate, start_col,1,end_col,3)
